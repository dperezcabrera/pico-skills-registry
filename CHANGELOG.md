...
### Fixed
- **Security: updating a skill no longer drops its access control.** A `PUT` payload that omitted `groups` silently reset them to empty, turning a group-gated skill public. Access metadata (`groups`, `tags`, `tools`) now persists across updates unless the payload explicitly changes it. Found by the 24-skill scale validation.
